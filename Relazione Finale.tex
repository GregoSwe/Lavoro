...
\begin{itemize}
	...
	\item \textbf{{\color{Plum} Applicativi Web-Based}:} \\
	 ...
	 \item \textbf{{\color{Plum} ATMEL:}}\\
	  società produttrice di semiconduttori con sede a San Jose, California (USA), fondata nel 1984 e specializzata nella realizzazione di microcontrollori di tipo system-on-a-chip con memoria flash integrata.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{{\color{OliveGreen}GNU GPL} (GNU General Public License) :} \\
	licenza per software libero, originariamente stesa nel 1989 da Richard Stallman per distribuire i programmi creati nell'ambito del Progetto GNU della \textit{Free Software Foundation} (FSF) per il progetto GNU;
\end{itemize}

\section*{I}
\begin{itemize}
	\item \textbf{{\color{OliveGreen}I2C} (Inter Integrated Circuit) :}\\
	 bus composto da almeno un master ed uno slave (letteralmente "capo, padrone" e "sottoposto, schiavo"), trattasi di un sistema di comunicazione seriale bifilare utilizzato tra circuiti integrati il cui vantaggio risiede nell'utilizzo di sole due linee di comunicazione, limitandone la velocità e distanza;
...
\begin{itemize}
	...
	\item \textbf{{\color{OliveGreen} Memoria SRAM  } (Static Random Access Memory):} \\
	 tipo di RAM volatile che non necessita di refresh. I banchi di memorie SRAM consentono di mantenere le informazioni per un tempo teoricamente infinito, hanno bassi tempi di lettura e bassi consumi, specialmente in condizioni statiche;
	
	\item \textbf{{\color{OliveGreen}MQTT} (MQ Telemetry Transport):}
	protocollo di messaggistica leggero posizionato in cima a TCP/IP disegnato per le situazioni in cui è richiesto un basso impatto e dove la banda è limitata.
\end{itemize}
...
\item \textbf{{\color{Plum}Reed}} \\
componente costituito da due lamine, realizzate con materiale ferromagnetico (una lega di ferro-nichel), parzialmente sovrapposte e separate tra loro di qualche decimo di millimetro. Sulle lamine contrapposte sono riportati dei contatti (generalmente in oro diffuso).
